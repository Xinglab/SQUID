...
options, args = getopt.getopt(sys.argv[1:], 'o:',['align=','GTF=','fastq=','check_len=','index_star=','index_kallisto=','l=','s=','output=','update=','lib=','read=','length=','anchor=','Cal=','FPKM=','Comparison=','analysis=','c1=','p=','resume=','F_deltaPI=','F_FDR='])
align ='';
GTF ='';
fastq='';
check_len='false';
...
if (run =="false"):
	...
	print "Usage :", sys.argv[0], " --F_FDR: The cutoff of combined FDR to output differential spliced introns.The default is 0.1;"
	...
	while(info2):
		...
		for i in range(0,num):
			...
			if((inc[i] + skp[i]) > 0) & ((float(inc[i])/in_l + float(skp[i])/sk_l) > 0):	
				PI_J[i]= str(round((float(inc[i])/in_l)/(float(inc[i])/in_l + float(skp[i])/sk_l),2))
			else:
				PI_J[i] ='NA'
		...
		for i in range(0, num):
			...
			exp[i] = a2[i+num+2]
				...
				PI_D[i] = str(round(float(a2[i +2])/int(a2[i+num+2]),2))
			else:
				PI_D[i] ='NA'
		info1 = fr1.readline()
		info2 = fr2.readline()
...
for info in fr:
	...
	fr1.close()
	fw.close()
	cmd ="bash %s/MATS/rMATS.sh -d %s/rMATS_files/rMATS_%s_Junction.txt -o %s/rMATS_files/rMATS_%s_Junction -p %s  -t %s -c %s" %(bin_path,output,a[0],output,a[0],p,analysis, c1)
	logging.debug(cmd)
	os.system(cmd)
	...
	##generate final output
		...
		logging.debug(cmd)
                os.system(cmd)
		cmd = "Rscript %s/RP_value.R %s/Diff_%s_intron_temp.txt 100 %s/rank_product_test_%s.txt %s/Result/Diff_%s_intron_PI.txt %s %s %s/Result/Increase_%s_intron_PI.txt %s/Result/Decrease_%s_intron_PI.txt" %(bin_path, output_S, a[0], output_S, a[0],output,a[0],F_deltaPI, F_FDR, output,a[0],output,a[0])                
		logging.debug(cmd)
                os.system(cmd)
		logging.debug("Output the result of differential spliced intron analysis of " + a[0] + "\n")
fr.close()
