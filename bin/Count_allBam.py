...
for file in bamfile:
	...
	for iter in fr2:
		...
		if ( fr2.getrname(iter.reference_id), index) in posE:
			for key in posE[fr2.getrname(iter.reference_id),index]:
				in_p = key.split("_")
				if ((int(in_p[1]) <= iter.get_reference_positions()[0]+1)& (int(in_p[2])>= iter.get_reference_positions()[0]+1)):
					if(lib =="unstrand"):
						Gene[EXON[key]][nn]+=1
				...
				if ( fr2.getrname(iter.reference_id),start+n1, start+n1+n2-1) in pp:
					if(lib =="unstrand"):
						count[pp[ fr2.getrname(iter.reference_id),start+n1,start +n1+n2 -1]][nn*6+4] +=1 
                                        elif(re.search(("\%s" %strand),count[pp[ fr2.getrname(iter.reference_id),start+n1,start +n1+n2 -1]][num*6+1])):
