...
##Intron adjust using strand info
bin = 1000
intron = dict()
intronJ = dict()
intronE = dict()
...
for info in fr:
        ...
        if(C1 =="false"):
                key ="%s_%s_%s" % (a[0],a[3],a[4])
                if( not intron.has_key(key)):
                        intron[key]= [a[0],int(a[3]),int(a[4]),a[6]] 
                        intronE[key] =[]
                        index_s =  int(a[3]) / bin
                        index_e =  int(a[4]) / bin
                        for i in  range(index_s , index_e+1):
                                pos_in.setdefault((a[0],a[6],i),[]).append(key)
...
for info in fr:
        ...
        for j in range(3, len(a)):
                ...
                if( not intron.has_key(a[j])):
                        ...
                        index_s =  int(in_p[1]) / bin
                        index_e =  int(in_p[2]) / bin
                        intron[a[j]]= [in_p[0], int(in_p[1]),int(in_p[2]),a[1]]
                        for i in  range(index_s , index_e+1):
                                pos_in.setdefault((a[0],a[1],i),[]).append(key)
fr.close()
##generate the dictionary with overlapping exons
fr= open(exon)
exon = dict()
for info in fr:
        ...
        index_e =  int(a[4]) / bin
        for i in  range(index_s , index_e+1):
                if(not pos_in.has_key((a[0],a[6],i,))):
                        continue
                for ii in pos_in[a[0],a[6],i]:
                        in_p = ii.split("_")
                        if(int(in_p[1]) > int(a[4])):
                                continue
                        elif( int(in_p[2]) < int(a[3])):
