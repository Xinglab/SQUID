...
if(lib=="unstrand"):
		...
		for iter in fr2:
			if(read =="P"):
				...
				if(iter.get_tag('NH')!=1):
					continue
				if(re.search("D",iter.cigarstring)):
					continue
	...
	for info1 in fr1:
		a1 = info1.strip().split("\t")
		key = "%s_%s_%s" % (a1[0],a1[3],a1[4])
		if(count[key][num*6+2]=="true"):
			if(re.search('\+',count[key][num*6+1])):
				fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (key, count[key][num*6],count[key][num*6+1],a1[0],a1[3],a1[4],"\t".join(str(x) for x in count[key][0:num*6])))
			else:
				fw.write("%s\t%s\t%s\t%s\t%s\t%s" % (key, count[key][num*6],count[key][num*6+1],a1[0],a1[3],a1[4]))
