...
options, args = getopt.getopt(sys.argv[1:], 'i:o:',['input=','output=','comparison=','p=','FDR=','Diff='])
input='';
output='.'
comparison=''
p = 1
...
if (not input or not comparison):
	...
	print "a python program to run rMATs from the counts files of IR and generate the final results"
	print "usage :", sys.argv[0], " -i/--input: input count files generated by IR program"
	print "usage :", sys.argv[0], " -o/--output: the output directory, default is current directory"
	print "usage :", sys.argv[0], " --comparison: a file providing the samples pairs need to be calculating the differential intron level"
	print "usage :", sys.argv[0], " --p: the number of threads used to run rMATS"
	print "usage :", sys.argv[0], " --FDR: the FDR to the significant differential RI events"
        print "usage :", sys.argv[0], " --Diff: the limit of minimal difference RI level"
...
for info in fr:
	...
	fr1.close()
	fw.close()
	cmd ="%s/MATS/rMATS.sh -d %s/rMATs_%s.txt -o %s/rMATs_%s -p %s  -c 0.0001" %(bin_path,output,a[0],output,a[0],p)
        print cmd
	os.system(cmd)
