...
4. Install [kallisto 0.43.0](https://pachterlab.github.io/kallisto/download) for the run without RPKM files provided. The kallisto index of human and mouse can be downloaded throught the following link
...
A: It depends on the input you used in the SQUID. If the input is alignment files and RPKM files, 4G is enough for the human genome. If RPKM is not provided, RPKM will be generated by Kallisto which may takes 1G extra RAM. If alignment files is not provided, STAR will generate alignment files that may takes up to 40G to for the human genome.
